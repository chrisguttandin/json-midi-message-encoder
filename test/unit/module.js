...
describe('module', () => {
    describe('encode()', () => {
        ...
        it('should encode a copyright notice event', () => {
            ...
            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 2, 49, 54, 47, 53, 47, 50, 48, 48, 51, 32, 98, 121, 32, 32, 65, 78, 84, 79, 78, 73, 79, 32, 76, 65, 86, 73, 65, 78, 79,
                32, 40, 109, 111, 110, 116, 101, 118, 105, 100, 101, 111, 32, 117, 114, 117, 103, 117, 97, 121, 41
            ]);
        });

        it('should encode a cue point event', () => {
            const arrayBuffer = encode({ cuePoint: 'a fake cue point', delta: 0 });

            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 7, 16, 97, 32, 102, 97, 107, 101, 32, 99, 117, 101, 32, 112, 111, 105, 110, 116
            ]);
        });

        it('should encode a device name event', () => {
            const arrayBuffer = encode({ deviceName: 'Steinway Grand Piano' });

            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 9, 20, 83, 116, 101, 105, 110, 119, 97, 121, 32, 71, 114, 97, 110, 100, 32, 80, 105, 97, 110, 111
            ]);
        });
        ...
        it('should encode a instrument name event', () => {
            ...
            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 4, 20, 83, 116, 101, 105, 110, 119, 97, 121, 32, 71, 114, 97, 110, 100, 32, 80, 105, 97, 110, 111
            ]);
        });
        ...
        it('should encode a program name event', () => {
            ...
            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 8, 19, 97, 32, 102, 97, 107, 101, 32, 112, 114, 111, 103, 114, 97, 109, 32, 110, 97, 109, 101
            ]);
        });
        ...
        it('should encode a track name event', () => {
            ...
            expect(arrayfy(arrayBuffer)).to.deep.equal([
                255, 3, 17, 97, 32, 102, 97, 107, 101, 32, 116, 114, 97, 99, 107, 32, 110, 97, 109, 101
            ]);
        });
